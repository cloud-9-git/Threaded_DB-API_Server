...
- 목적: `workers=4`, `queue=32`부터 시작해 조합을 넓게 탐색하고, 시나리오별로 적절한 워커/큐 조합을 찾는다.
...
- 반복 측정 수: 요약표 기준 `177`개 조합, 개별 실행 기록 기준 `1770`회 실험
...
- 짧은 순간 몰림(`burst 1024`)에서는 워커 수보다 `queue` 크기 영향이 더 컸습니다. 특히 `100k`에서는 `4/512`가 `8/256`, `16/256`보다 확실히 안정적이었습니다.
...
| 평상시 트래픽 (steady-state, 동시 128) | 1000000 | `8/128` | p95=`15.231`, 표준편차=`0.810` | `8/128` | 100만 건 데이터에서는 `8/128`이 가장 안정적이었습니다. `4/128`도 버티기는 했지만 느린 요청이 더 길었습니다. |
...
| 짧은 순간 몰림 (burst 1024) | 10000 | `16/128` | p95=`6.777`, 표준편차=`11.520` | `16/128` | 1만 건 데이터에서 순간적으로 1024개 요청이 몰릴 때는 완전히 안정적인 조합이 많지 않았고, 그중 `16/128`이 가장 나았습니다. |
...
| 짧은 순간 몰림 (burst 1024) | 100000 | `4/512` | p95=`7.828`, 표준편차=`12.709` | `4/512` | 10만 건 데이터에서 순간적으로 1024개 요청이 몰릴 때는 큐 크기가 특히 중요했습니다. `4/512`가 가장 안정적이었습니다. |
...
| 짧은 순간 몰림 (burst 1024) | 1000000 | `8/512` | p95=`4.439`, 표준편차=`3.859` | `4/256` | 100만 건 데이터에서 순간적으로 1024개 요청이 몰릴 때는 `8/512`가 가장 좋았습니다. 다만 `4/256`도 성능 차이 없이 더 가벼운 절충안이었습니다. |
...
- 평상시 트래픽에서는 `4/128`이 1만 건과 10만 건 데이터의 공통 안정 지점이었습니다. 이 구간에서는 워커를 더 늘려도 얻는 이익보다 결과 흔들림이 먼저 커졌습니다.
- 100만 건 데이터의 평상시 트래픽에서는 워커를 `8`까지 늘리는 편이 유리했습니다. 데이터가 커질수록 큐보다 워커 수가 더 중요해지는 구간이 생겼습니다.
- 10만 건 데이터에서 `burst 1024`를 걸면, 워커 수보다 큐 크기의 영향이 더 컸습니다. `4/512`가 `8/256`, `16/256`보다 낫다는 점이 그 증거입니다.
- 100만 건 데이터에서 `burst 1024`를 걸면 `8/512`가 가장 빠르지만, `4/256`도 거의 같은 성능이라서 비용 대비 효율이 높았습니다.
...
```mermaid
xychart-beta
    title "10만 건 데이터 / 초고동시 접속 20000 / queue 512 : worker 수별 accepted 평균"
    x-axis ["4", "8", "16"]
    y-axis "success_mean" 0 --> 600
    bar [516.0, 520.0, 528.0]
```

## 결과 해석 메모

- `4/128`은 1만 건과 10만 건 데이터의 평상시 운영 기본값으로 가장 설득력이 높았습니다.
- `8/128`은 100만 건 데이터의 평상시 운영 기본값으로 가장 무난했습니다. `16/64`도 좋았지만, 워커를 더 많이 쓰는 이점이 크지 않았습니다.
- `8/64`는 10만 건 데이터에서 `burst 512` 상황을 가장 잘 버틴 절충안이었습니다.
- `4/512`는 10만 건 데이터에서 `burst 1024` 상황에 가장 강했습니다. 이 경우에는 워커 수보다 큐 크기가 더 중요했습니다.
- `8/512`는 100만 건 데이터의 `burst 1024`와 초고동시 접속 시험에서 공통으로 쓸 만한 절충안이었습니다. 최대치가 꼭 필요하면 `16/512`, 비용 대비 효율을 보려면 `8/512`가 더 나았습니다.
