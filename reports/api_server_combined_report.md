...
- 맞습니다. 이전 버전의 `combined`에서는 `성공률`, `p95`, `표준편차`가 앞에 나오고, `처리속도`는 눈에 덜 띄었습니다.
- 이 문서에서 말하는 `처리속도`는 보통 `성공 처리량(req/s)`입니다.
  - 뜻: 1초에 성공적으로 끝낸 요청 수
  - 계산: `성공 요청 수 / 전체 소요 시간`
- 다만 이번 보고서에서는 처리속도 하나만으로 조합을 고르지 않았습니다.
  - 처리속도가 높아도 `503`이 많거나 `p95`가 나쁘면 실제 운영 체감은 좋지 않을 수 있기 때문입니다.
  - 그래서 `처리속도`는 참고 지표로 보고, 최종 선택은 `실패율 + p95 + 표준편차`를 더 우선했습니다.

| 대표 시나리오 | 권장 조합 | 성공 처리량(req/s) | 함께 봐야 할 값 |
| --- | --- | --- | --- |
| 평상시 10k (`steady-state 128`) | `4/128` | `6273.7` | p95=`16.372` |
| 평상시 100k (`steady-state 128`) | `4/128` | `5609.1` | p95=`18.732` |
| 평상시 1M (`steady-state 128`) | `8/128` | `6272.7` | p95=`15.231` |
| 순간 몰림 10k (`burst 512`) | `16/256` | `5628.8` | p95=`3.078` |
| 순간 몰림 10k (`burst 1024`) | `2/256` | `5006.7` | p95=`3.890` |
| 순간 몰림 100k (`burst 512`) | `8/64` | `4808.9` | p95=`3.718` |
| 순간 몰림 100k (`burst 1024`) | `1/128` | `4581.8` | p95=`7.306` |
| 순간 몰림 1M (`burst 512`) | `16/32` | `5655.9` | p95=`3.215` |
| 순간 몰림 1M (`burst 1024`) | `2/32` | `4862.9` | p95=`4.166` |
| 초고동시 100k (`async 2000`) | `16/512` | `2452.9` | accepted=`528`, p95=`206.586` |
| 초고동시 100k (`async 5000`) | `16/512` | `900.1` | accepted=`528`, p95=`565.002` |
| 초고동시 100k (`async 10000`) | `16/512` | `421.7` | accepted=`528`, p95=`1209.274` |
| 초고동시 1M (`async 10000`) | `16/512` | `418.2` | accepted=`528`, p95=`1218.437` |

## 처리속도 읽는 법

- `steady-state`에서 처리속도는 `평소 운영에서 얼마나 꾸준히 일을 처리하는지`를 보여줍니다.
- `burst`에서 처리속도는 `순간 몰림을 얼마나 빨리 흡수하는지`를 보여줍니다.
- `async`에서 처리속도는 해석을 조금 조심해야 합니다.
  - 여기서는 서버가 일부 요청만 받아들이고 나머지를 빨리 거절하기 때문에, `req/s` 하나만 보면 실제 수용량 한계가 가려질 수 있습니다.
  - 그래서 `async`에서는 `처리속도`와 함께 반드시 `accepted`, `503`, `p95`를 같이 봐야 합니다.
- 이번 결과에서 중요한 포인트는 이렇습니다.
  - `burst`에서는 낮은 워커 수가 의외로 좋은 처리속도와 좋은 p95를 함께 만든 구간이 있었습니다.
  - 하지만 `async`에서는 `worker=1/2`가 받아낼 수 있는 요청 수 자체가 너무 작아서, 처리속도만 좋아 보여도 운영 기본값으로는 불리했습니다.
