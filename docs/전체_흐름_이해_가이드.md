...
```mermaid
flowchart LR
    Client["클라이언트<br/>curl / 브라우저 / 테스트 스크립트"]
    Server["mini_db_server<br/>새로 추가할 서버"]
    HTTP["http.c<br/>HTTP 요청/응답"]
    JSON["json_parser.c / json_writer.c"]
    DBAPI["db_api.c<br/>서버-엔진 연결 계층"]
    Lexer["lexer.c"]
    Parser["parser.c"]
    Executor["executor.c"]
    Runtime["runtime.c"]
...
아래 그림에서 왼쪽은 새로 붙는 서버 레이어이고, 오른쪽은 기존 엔진이다.

```mermaid
flowchart LR
    subgraph ServerLayer["새로 추가할 서버 레이어"]
        ServerMain["src/server_main.c"]
        Server["src/server.c"]
        ThreadPool["src/thread_pool.c"]
        Queue["src/task_queue.c"]
        HTTP["src/http.c"]
        JsonParser["src/json_parser.c"]
        JsonWriter["src/json_writer.c"]
        DBAPI["src/db_api.c"]
    end

    subgraph EngineLayer["기존 SQL 엔진"]
        Lexer["src/lexer.c"]
        Parser["src/parser.c"]
        Executor["src/executor.c"]
        Runtime["src/runtime.c"]
